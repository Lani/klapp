  ...
  return (
    <div class="screens-list">
      ...
      <For each={props.screens}>
        {(screen) => (
          <div class={`screen-item ${props.activeScreenId === screen.id ? "active" : ""}`}>
            <Show 
              ...
              fallback={
                <div style={{ "display": "flex", "width": "100%" }}>
                  <input
                    ...
                    autofocus
                  />
                  <button onClick={saveEditing} style={{ "margin-left": "5px" }}>✓</button>
                  <button onClick={() => setEditingId(null)} style={{ "margin-left": "5px" }}>✗</button>
                </div>
              }
            >
              <div 
              ...
              <div class="screen-actions">
                <button onClick={() => startEditing(screen.id, screen.name)}>✏️</button>
                <button onClick={() => handleDeleteScreen(screen.id)}>🗑️</button>
              </div>
            </Show>
          </div>
        )}
        ...
        fallback={
          <div style={{ "margin-top": "10px", "display": "flex" }}>
            <input
              ...
              autofocus
            />
            <button onClick={handleAddScreen} style={{ "margin-left": "5px" }}>Add</button>
            <button onClick={() => setIsAdding(false)} style={{ "margin-left": "5px" }}>Cancel</button>
          </div>
        }
      >
        <button 
