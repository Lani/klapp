...
const App: Component = () => {
  const [screens, setScreens] = createStore<
...
export default function ${name}() {
  ...
  const updateComponentProperty = (property: string, value: any) => {
    ...
    const activeScreenData = screens.find(
      (s: { id: string }) => s.id === activeScreen(),
    );
    if (activeScreenData) {
                ...
                const buttonElement = recast.types.builders.jsxElement(
                  ...
                  [
                    recast.types.builders.jsxText(
                      newComponent.props.children || 'Button',
                    ),
                  ],
                );
    ...
    <div class={styles.appBuilder}>
      <DragDropProvider>
        <DragDropSensors>
          ...
          <DragOverlay>
            {draggingItem() && (
              <div class="toolbox-item-overlay">{draggingItem().type}</div>
            )}
          </DragOverlay>
        </DragDropSensors>
      </DragDropProvider>
